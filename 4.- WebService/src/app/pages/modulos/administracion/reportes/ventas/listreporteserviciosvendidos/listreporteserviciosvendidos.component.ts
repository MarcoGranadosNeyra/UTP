...
@Component({
  selector: 'app-listreporteserviciosvendidos',
  templateUrl: './listreporteserviciosvendidos.component.html',
  styleUrls: ['./listreporteserviciosvendidos.component.css']
})
export class ListreporteserviciosvendidosComponent implements OnInit {
  ...
  displayedColumns: string[] = ['SERVICIO','CANTIDAD','MONTO'];
  ...
  if(this.formularioFechas.valid){
      this.ventaService.serviciosVendidos(this.formularioFechas.value)
      .subscribe(res => {
        this.dataArray = res;
  ...
  descargarDocumento() {
    ...
    const pdfUrl = `${this.apiURL}/venta/reporteServiciosVendidosPDF/${fecha1}/${fecha2}`;
    const pdfName = 'ReporteServicios_'+fecha1+fecha2;
  ...
  abrirDocumento() {
    var fecha1=this.formularioFechas.get('fecha1').value;
    var fecha2=this.formularioFechas.get('fecha2').value;
    window.open(`${this.apiURL}/venta/reporteServiciosVendidosPDF/${fecha1}/${fecha2}`+ '#page=' + 1, '_blank', '');
  }
